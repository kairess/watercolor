...
bool haveColorDifference(Mat &dis, Mat &meanshift, int x, int y)
{
	...
	{
		for (int j = y - size; j <= y + size; j++)
			if (ArraySpace::inMap(dis, myPoint(i, j)))
			{
				if (dataAt<uchar>(meanshift, x, y) != dataAt<uchar>(meanshift, i, j) &&
...
void Abstraction::deal(Mat &src, Mat &mySaliency, Mat &myDis, Mat &dst)
{
	...
	for (int i = 0; i < ref.rows; i++)
	{
		for (int j = 0; j < ref.cols; j++)
		{
			...
			{
				for (int y = j - size1; y <= j + size1; y++)
					if (ArraySpace::inMap(ref, x, y))
					{
			...
			if (dataAt<uchar>(mySaliency, i, j) == 0)
			{
				...
				{
					for (int y = j - size2; y <= j + size2; y++)
						if (ArraySpace::inMap(ref, myPoint(x, y)))
						{
							if (dataAt<uchar>(myMeanShift, x, y) != dataAt<uchar>(myMeanShift, i, j) && abs(dataAt<float>(myDis, x, y) - dataAt<float>(myDis, i, j)) < 0.3 * d)
							{
